        ...
        {    
            foreach (var warp in Object.FindObjectsByType<WarpPos>(FindObjectsSortMode.None))
            {
                ...
                if (warp.GetComponent<WarpPos>().WarpPosID == warpPosID)
                    warpPos = warp.GetComponent<WarpPos>().warpPos.transform.position;
            }
        }
        */
        var objs = Object.FindObjectsByType<GameObject>(FindObjectsSortMode.None);
        Debug.Log($"Found {objs.Length} WarpPos objects.");

        foreach (var obj in objs)
        {
            if (obj.GetComponent<WarpPos>() != null)
            {    
                Debug.Log($"Checking WarpPos with ID {obj.GetComponent<WarpPos>().WarpPosID}");
                if (obj.GetComponent<WarpPos>().WarpPosID == warpPosID)
                {
