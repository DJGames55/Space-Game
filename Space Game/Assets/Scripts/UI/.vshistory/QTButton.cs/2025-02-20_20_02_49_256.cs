...
public class QTButton : MonoBehaviour
{
    ...
    private void GetWarpPos()
    {
        ...
        */
        var warps = Object.FindObjectsByType<WarpPos>(FindObjectsSortMode.None);
        Debug.Log($"Found {warps.Length} WarpPos objects.");

        foreach (var warp in warps)
        {
            Debug.Log($"Checking WarpPos with ID {warp.warpPosID}");
            if (warp.warpPosID == warpPosID)
            {
