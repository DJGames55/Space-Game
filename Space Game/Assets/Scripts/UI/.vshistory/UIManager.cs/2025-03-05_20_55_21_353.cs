            ...
            foreach (Transform child2 in child.transform)
            {
                if (child.GetComponent<QTButton>() != null && !child.GetComponent<QTButton>().currentlyDisabledInScene)
                {
                    ...
                    if (QTbutton.starTravel)
                        distanceToPos = Vector3.Distance(_ship.gameObject.transform.position, QTbutton.starLocation);
                    ...
                    if (QTbutton.warpPos != null)
                        Debug.Log($"{child} + {QTbutton.warpPos.transform.position}");
                    else
                        Debug.Log($"{child} + {QTbutton.starLocation}");
                }
            }
        }
    }
