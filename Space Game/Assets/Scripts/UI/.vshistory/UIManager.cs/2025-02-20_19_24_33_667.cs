...
public class UIManager : MonoBehaviour
{
        ...
        { 
            foreach (Transform child2 in child.transform)
            {
                if (child.GetComponent<QTButton>() != null && !child.GetComponent<QTButton>().currentlyDisabledInScene)
                {
                    ...
                    {
                        distanceToPos = Vector3.Distance(_ship.gameObject.transform.position, QTbutton.warpPos);
                    }
    ...
    public float GetStarDistance(SceneField targetStar)
    {
        ...
        {
            if (var.fromStar.SceneName == currentScene.SceneName && var.toStar.SceneName == targetStar.SceneName)
            {
                float distanceToStar = (Vector3.Distance(_ship.gameObject.transform.position, var.distance) * 1_000_000_000_000_000);
