...
public class UIManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        **/
        ...
        if (QTMenu == null)
            Debug.LogWarning("no QTMenu");
        else
            Debug.Log("yes QTMenu");
    }
    ...
    {
        if (QTMenu.transform != null)
        {
            foreach (Transform child in QTMenu.transform)
            {
                foreach (Transform child2 in child.transform)
                {
                    if (child.GetComponent<QTButton>() != null && !child.GetComponent<QTButton>().disabledInScene)
                    {
                        ...
                        {
                            distanceToPos = GetStarDistance(QTbutton.starScene);
                        }
    ...
    public float GetStarDistance(SceneField targetStar)
    {
        foreach (StarLocations.DistanceData data in _starLocData.savedDistances)
        {
            if (data.fromStar.SceneName == currentScene.SceneName && data.toStar.SceneName == targetStar.SceneName)
            {
                float distanceToStar = (Vector3.Distance(_ship.gameObject.transform.position, data.distance) * 1_000_000_000_000_000);
