...
public class QTButton : MonoBehaviour
{
    ...
    public void GetWarpPos()
    {
        foreach (WarpPos warp in Object.FindObjectsByType<WarpPos>(FindObjectsSortMode.None))
        {
            ...
            if (warpPosData.WarpID == warpID)
            {
                if (warpPosData.WarpID != "Asteroid1")
                    warpPosData.gameObject.SetActive(false);

                warpPos = warpPosData.WarpLoc;

                if (warpPosData.WarpPosition != null)
                    warpPos = warpPosData.WarpPosition.gameObject;
