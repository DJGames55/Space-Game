...
public class AsteroidBehaviour : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            if (impactEffect != null) // Check if impactEffect is assigned
            {
                ParticleSystem impact = Instantiate(impactEffect, transform.position, Quaternion.identity);
                Destroy(impact.gameObject, 2f); // Destroy after 2 seconds
            }
            else
            {
                Debug.LogWarning("Impact effect is not assigned in the Inspector!");
            }
