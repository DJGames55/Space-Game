...
public class ShipControls : MonoBehaviour
{
    ...
    public IEnumerator QT(GameObject travelLocation)
    {
        ...
        float distanceToWarp = Vector3.Distance(playerShip.transform.position, travelLocation.transform.position);
        while ( distanceToWarp >= 2f )
        {
            float speed = 5f; // The speed at which the ship moves
            playerShip.transform.position = Vector3.Lerp(playerShip.transform.position, travelLocation.transform.position, speed * Time.deltaTime);
            Debug.Log("ballz");
        }
