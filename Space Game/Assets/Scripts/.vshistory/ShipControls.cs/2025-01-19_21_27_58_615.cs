...
public class ShipControls : MonoBehaviour
{
    ...
    private bool isQT;
    public float QTMoveSpeed = 5f; // Speed of movement
    private float lerpTime = 0f; // Timer to track how much time has passed
    public float journeyLength = 10f; // Total time or distance for the move
    public GameObject travelPos;
    ...
    private void Update()
    {
        ...
        if (isQT)
        {
            // Increment the lerpTime by the time passed in the current frame
            lerpTime += Time.deltaTime * QTMoveSpeed;

            // Normalize lerpTime to go from 0 to 1
            float t = Mathf.Clamp01(lerpTime / journeyLength);

            // Smoothly move the ship to the travel location
            playerShip.transform.position = Vector3.Lerp(playerShip.transform.position, travelPos.transform.position, t);
        }
    }
    ...
    public IEnumerator QT(GameObject travelLocation)
    {
        ...
        while (playerShip.transform.position != travelLocation.transform.position)
        {
            isQT = true;
        }
        isQT = false;
