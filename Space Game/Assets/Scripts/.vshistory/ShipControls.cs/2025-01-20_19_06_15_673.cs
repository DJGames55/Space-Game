...
public class ShipControls : MonoBehaviour
{
    ...
    public IEnumerator QT(GameObject travelLocation)
    {
        ...
        while (QTSlider.value < 0.999f) // Avoid exact float comparison
        {
            QTSlider.value = Mathf.Lerp(QTSlider.value, 1, 2 * Time.deltaTime);
            yield return null;
        }
        QTSlider.value = 1; // Ensure it reaches exactly 1
