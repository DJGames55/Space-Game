...
public class ShipControls : MonoBehaviour
{
    ...
    public IEnumerator QT(GameObject travelLocation)
    {
        ...
        // Wait before moving the playerShip
        yield return new WaitForSeconds(1f);

        // Move the playerShip to the travel location
        playerShip.transform.position = travelLocation.transform.position;

        playerShip.transform.position = Vector3.Lerp(playerShip.transform.position, travelLocation.transform.position, Time.deltaTime * speed);
