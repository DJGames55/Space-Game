...
public class ShipControls : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            QTParticles.time = 5;
            StartCoroutine(QTToCurrentLoc(FindSceneLastLoc()));
            isStarQT = false;
        }
    ...
    private IEnumerator StarQT(Vector3 starLocation, SceneField starScene, string warpName)
    {
        ...
        while (warpTime < 3f)
        {
            ...
            if (warpTime < 3f)
                playerShip.transform.position = Vector3.Lerp(playerShip.transform.position, (starLocation / 1_000_000_000), QTSpeed * Time.deltaTime);
    ...
    public IEnumerator QTToCurrentLoc(Vector3 prevSceneLoc)
    {

        float warpTime = 0;
        float startFOV = MainCam.fieldOfView;
        float endFOV = 167f;
        float starQTWarpTime = 5f;
