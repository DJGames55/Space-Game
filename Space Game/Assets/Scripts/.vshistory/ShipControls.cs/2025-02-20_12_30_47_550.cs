...
public class ShipControls : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            QTParticles.time = 5;
            StartCoroutine(QTToCurrentLoc(FindSceneLastLoc(), prevScene));
            isStarQT = false;
        }
    ...
    public IEnumerator QTToCurrentLoc(Vector3 prevSceneLoc, SceneField prevScene)
    {
        ...
        {
            if (var.toStar.SceneName == prevScene.SceneName && var.fromStar == _ui.currentScene)
            {
                transform.position = var.distance * 1_000_000;
            }
        }
        ...
        while (warpTime < totalWarpTime)
        {
            MainCam.fieldOfView = Mathf.Lerp(e, endFOV, warpTime / totalWarpTime);
