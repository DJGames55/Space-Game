...
public class ShipControls : MonoBehaviour
{
    ...
    [SerializeField] private Camera QTCam;
    private Coroutine QTCoroutine;
    private bool warping;
    private bool loadingQT;
    public static bool isStarQT;
    ...
    private void Start()
    {
        ...
        QTCam.gameObject.SetActive(false);
    }
    ...
    public void InitiateQT(GameObject button)
    {
        ...
        Transform travelLocation = QTVar.warpPos;
        string warpName = QTVar.warpName;

        bool starTravel = QTVar.starTravel;
        Vector3 starLocation = QTVar.distanceOverride;
        SceneField starScene = QTVar.starScene;

        StartQT(travelLocation, warpName, starTravel, starLocation, starScene);
    }

    public void StartQT(Transform travelLocation, string warpName, bool starTravel, Vector3 starLocation, SceneField starScene)
    {
    ...
    public IEnumerator QTStart(Transform travelLocation, string warpName, bool starTravel, Vector3 starLocation, SceneField starScene)
    {
        _ui.CloseQT();
        _input.SetUI();
    ...
    private IEnumerator QT(Transform travelLocation, string warpName)
    {
        loadingQT = true;
    ...
    private IEnumerator StarQT(Vector3 starLocation, SceneField starScene, string warpName)
    {
        ...
        {
            QTSlider.value += Time.deltaTime; // Moves at a constant rate
            QTCam.fieldOfView = Mathf.Lerp(startFOV, endFOV, warpTime / QTSlider.maxValue);
            yield return null;
        }
        QTSlider.value = 0;
        QTSlider.gameObject.SetActive(false);
        ...
        QTCam.gameObject.SetActive(true);
    ...
    public IEnumerator QTToCurrentLoc()
    {
        warping = true;
        ...
        float startFOV = MainCam.fieldOfView;
        float endFOV = 167f;
        float starQTWarpTime = 10f;
        while (warpTime < starQTWarpTime)
        {
            warpTime += Time.deltaTime;
            QTCam.fieldOfView = Mathf.Lerp(startFOV, endFOV, warpTime / starQTWarpTime);
            yield return null;
        }
