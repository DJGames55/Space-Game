...
public class ShipControls : MonoBehaviour
{
    ...
    [SerializeField] private SceneField prevScene;
    ...
    private void Awake()
    {
        if (isStarQT)
        {
            QTToCurrentLoc(FindSceneLastLoc(), prevScene);
            isStarQT = false;
        }
    ...
    public void QTToCurrentLoc(Vector3 prevSceneLoc, SceneField prevScene)
    {
        Debug.Log(prevScene.SceneName);
        ...
        while (warpTime < totalWarpTime)
        {
            //MainCam.fieldOfView = Mathf.Lerp(startFOV, endFOV, 1 / totalWarpTime);
