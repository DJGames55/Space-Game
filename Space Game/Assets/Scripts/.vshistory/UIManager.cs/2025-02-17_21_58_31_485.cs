    ...
    public void UpdateQTDistance()
    {
        foreach(Transform child in QTMenu.transform)
        {
            foreach(Transform child2 in child.transform)
            {
                ...
                {
                    float distanceToPos = child.GetComponent<QTButton>().distanceOverride;
                }
                else
                {
                    float distanceToPos = Vector3.Distance(_ship.transform.position, child.GetComponent<QTButton>().warpPos.transform.position);
                }
