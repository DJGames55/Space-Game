...
public class UIManager : MonoBehaviour
{
    ...
    public void UpdateQTDistance()
    {
        foreach(Transform child in QTMenu.transform)
        {
            foreach(Transform child2 in child.transform)
            {
                ...
                float distanceToPos = (Vector3.Distance(_ship.transform.position, child.GetComponent<QTButton>().warpPos.transform.position)) / 1000;                

                text.text = child.GetComponent<QTButton>().warpName.transform.position + " - " + FormatDistance(distanceToPos); ;
            }
        }
    }
    ...
    {
        if (distance >= 1_000_000_000f) // >= 1 Gm
