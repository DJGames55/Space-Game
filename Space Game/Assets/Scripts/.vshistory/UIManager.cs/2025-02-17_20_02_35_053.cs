    ...
    public void UpdateQTDistance()
    {
        foreach (Transform child in QTMenu.transform)
        {
            foreach (Transform child2 in child.transform)
            {
                ...
                QTButton button = child.GetComponent<QTButton>();
                if (button != null && button.warpPos != null)
                {
                    float distance = Vector3.Distance(transform.position, button.warpPos.position);
                    string formattedDistance = FormatDistance(distance);
                    text.text = $"{currentText} {formattedDistance}";
                }
                else
                {
                    text.text = $"{currentText} N/A";
                }
            }
        }
    }
