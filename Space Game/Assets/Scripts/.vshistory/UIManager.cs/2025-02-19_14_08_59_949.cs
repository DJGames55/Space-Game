...
public class UIManager : MonoBehaviour
{
    ...
    public void UpdateQTDistance()
    {
        foreach(Transform child in QTMenu.transform)
        {
            ...
            {
                if (child.GetComponent<QTButton>() != null && !child.GetComponent<QTButton>().diabledInScene)
                {
                    float distanceToPos;
                    QTButton QTbutton = child2.GetComponent<QTButton>();

                    if (child.GetComponent<QTButton>().overrideDistance)
                    {
                        distanceToPos = Vector3.Distance(_ship.transform.position, QTbutton.distanceOverride);
                    }
                    else
                    {
                        if (QTbutton.warpPos != null)
                        {
                            distanceToPos = Vector3.Distance(_ship.transform.position, QTbutton.warpPos.transform.position);
                        }
                        else
                        {
                            distanceToPos = 0;
                            Debug.LogError($"{child} Does not have a Warp Position specified. You may need to set overrideDistance to True/Disable Button");
                        }
                    }
