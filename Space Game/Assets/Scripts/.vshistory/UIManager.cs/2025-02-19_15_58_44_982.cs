        ...
        foreach(Transform child in QTMenu.transform)
        {
            foreach(Transform child2 in child.transform)
            {
                if (child.GetComponent<QTButton>() != null && !child.GetComponent<QTButton>().diabledInScene)
                {
                    ...
                    if (QTbutton.overrideDistance)
                    {
                        Debug.Log($"{child} - {QTbutton.overrideDistance}, {QTbutton.distanceOverride}");
                        distanceToPos = Vector3.Distance(_ship.gameObject.transform.position, QTbutton.distanceOverride);
                    }
                    else if (QTbutton.warpPos != null)
                    {
                        Debug.Log($"{child} - {QTbutton.overrideDistance}, {QTbutton.warpPos}");
                        distanceToPos = Vector3.Distance(_ship.gameObject.transform.position, QTbutton.warpPos.position);
                    }
