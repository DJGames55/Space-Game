...
public class AsteroidSpawner : MonoBehaviour
{
    ...
    [ContextMenu("Populate Asteroids")]
    private void PopulateArea()
    {
        ...
        while (attempts < maxAttempts)
        {
            ...
            if (validPosition)
            {
                ...
                int spawnNum = UnityEngine.Random.Range(0, 10);
                Asteriods asteroid = asteroidPrefabs[UnityEngine.Random.Range(0, asteroidPrefabs.Length)];
                while (asteroid.SpawnPriority > spawnNum)
                {
                    asteroid = asteroidPrefabs[UnityEngine.Random.Range(0, asteroidPrefabs.Length)];
                }
                
                GameObject selectedPrefab = asteroid.Asteroid;
