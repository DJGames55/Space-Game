...
public class QTControls : MonoBehaviour
{
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        foreach (QTButton qtButton in Object.FindObjectsByType<QTButton>(FindObjectsSortMode.None))
        {
            qtButton.GetComponent<QTButton>().GetWarpPos();
        }
    }

    private void StartingLoc()
    {
        foreach (WarpPos warp in Object.FindObjectsByType<WarpPos>(FindObjectsSortMode.None))
        {
            WarpPos warpPosData = warp.GetComponent<WarpPos>();

            if (warpPosData.WarpID == "Asteroid1")
            {
                if (warpPosData.WarpLoc != null)
                    warpPosData.WarpLoc.gameObject.SetActive(true);
                
                lastLoc = warpPosData.WarpLoc;
                return;
            }
        }
    }
    ...
    public void InitiateQT(GameObject button)
    {
        ...
        GameObject travelLocation = null;
    ...
    public IEnumerator QTStart(GameObject travelLocation, string warpName, bool starTravel, Vector3 starLocation, SceneField starScene)
    {
        ...
        if (travelLocation != null && !starTravel)
        {
    ...
    private IEnumerator QT(GameObject travelLocation, string warpName)
    {
        ...
        nextQTLoc = travelLocation.gameObject;
        elapsedTime = 0f;
        float currentSpeed = 0f;
        float maxSpeed = QTSpeed * 1000f;

        Vector3 startPos = playerShip.transform.position;
        Vector3 targetPos = travelLocation.transform.position;
        float totalDistance = Vector3.Distance(startPos, targetPos);
        float halfDistance = Vector3.Distance(nextQTLoc.transform.position, lastLoc.transform.position) / 2;
        float decelDistance = 0.5f * maxSpeed * QTAccel; // Distance required to decelerate smoothly

        while (Vector3.Distance(playerShip.transform.position, targetPos) > 5f)
        {
            float remainingDistance = Vector3.Distance(playerShip.transform.position, targetPos);

            // Acceleration Phase
            if (elapsedTime < QTAccel && remainingDistance > decelDistance)
            {
                currentSpeed = Mathf.Lerp(0, maxSpeed, elapsedTime / QTAccel);
                elapsedTime += Time.deltaTime;
            }
            // Deceleration Phase
            else if (remainingDistance <= decelDistance)
            {
                float decelTimeElapsed = QTAccel - (remainingDistance / decelDistance) * QTAccel;
                currentSpeed = Mathf.Lerp(maxSpeed, 0, decelTimeElapsed / QTAccel);
            }
            ...
            // Move the ship
            playerShip.transform.position = Vector3.MoveTowards(playerShip.transform.position, targetPos, currentSpeed * Time.deltaTime);

            // Enable nextQTLoc and disable lastLoc at halfway point
            if (Vector3.Distance(playerShip.transform.position, targetPos) < halfDistance)
            {
                nextQTLoc.SetActive(true);
                lastLoc.SetActive(false);
            }
