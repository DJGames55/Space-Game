...
public class QTControls : MonoBehaviour
{
    ...
    private IEnumerator QT(GameObject travelLocation, string warpName)
    {
        ...
        {
            playerShip.transform.position = Vector3.Lerp(playerShip.transform.position, travelLocation.transform.position, * Time.deltaTime);
            if (Vector3.Distance(playerShip.transform.position, travelLocation.transform.position) < Vector3.Distance(nextQTLoc.transform.position, lastLoc.transform.position))
            {
